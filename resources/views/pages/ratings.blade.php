...
{{-- <body class="bg-gray-100"> --}}
    ...
    <!-- Judul Tabel -->
    <div class="px-6 py-4 border-b">
        <h2 class="text-2xl font-bold text-gray-900">Daftar Ulasan Pengguna</h2>
    </div>
    ...
    <div class="overflow-x-auto">
        <table class="w-full">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @foreach ($rewiews as $review)
                    <tr class="hover:bg-gray-50">
                        <td class="px-6 py-4 text-sm text-gray-900">{{ $review->id }}</td>
                        <td class="px-6 py-4 text-sm text-gray-900">{{ $review->user->name ?? 'Tidak diketahui' }}</td>
                        <td class="px-6 py-4 text-sm text-gray-900">{{ $review->destination->name ?? 'Tidak diketahui' }}</td>
                        <td class="px-6 py-4 text-sm text-gray-900">{{ $review->rating }}/5</td>
                        <td class="px-6 py-4 text-sm text-gray-900">{{ Str::limit($review->comment, 50) }}</td>
                        <td class="px-6 py-4 text-sm text-gray-900">{{ \Carbon\Carbon::parse($review->created_at)->format('d-m-Y') }}</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
